...
def main(args: argparse.Namespace, use_checkpoint=True) -> None:
    ...
    #  if use_checkpoint:
    #  checkpoint_path = (
    #  "/home/epiphyte/Documents/Research/efficient_3d_Det/checkpoints/model00"
    #  )
    ...
    # Init Network
    net = MobileSal(mobile_net_settings).to(device)
    # net.load_state_dict(checkpoint["model_state_dict"])
    net.apply(init_weights)

    solver = utils.Solver(net)

    loss = solver(train_set=dataloader, validate_set=dataloader, epochs=60)
    plotloss(loss, 1)
